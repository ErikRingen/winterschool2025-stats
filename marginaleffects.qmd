...
Quarto markdown for this session here:
...
![](figures/hall_of_mirrors.png){width="65%"}
...
```{r, echo = T}
#| out.width: "70%"
plot_predictions(logistic_model, condition = list("condition")) + theme_classic(base_size = 24) + ylab("Pr(Agent)")
```
...
#| out.width: "60%"
library(mgcv)
gam_model <- mgcv::gam(structural_diversity ~ s(log_density))
...
![](figures/time_allocation.png){width="75%"}
...
- Automatically detects dependencies between parts of your pipeline, which means:
    - If you change some part of your pipeline, `targets` will rerun (only) the parts that depend on that part.

## Use one of the synthetic datasets from Part 1
...
## See what happens if we change the plot
