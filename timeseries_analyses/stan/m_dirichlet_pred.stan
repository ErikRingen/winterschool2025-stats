...
parameters{
  ...
  matrix[5, K - 1] b; // fixed effects
  
  // random effects
  matrix[7 * (K - 1), N_species] species_z; 
  matrix[6 * (K - 1), N_pid] pid_z;
  
  matrix[(K - 1), N_stimuli] stimulus_z;
  matrix[(K - 1), N_fs] fs_z;
  matrix[(K - 1), N_species * N_fs] fs_species_z;
  
  // variance components
  vector<lower=0>[7 * (K - 1)] sigma_species;
  vector<lower=0>[6 * (K - 1)] sigma_pid;
  vector<lower=0>[K-1] sigma_stimulus;
  vector<lower=0>[K-1] sigma_fs;
  vector<lower=0>[K-1] sigma_fs_species;
}
...
transformed parameters{
  ...
  matrix[N_species, 7 * (K - 1)] species_v = (diag_matrix(sigma_species) * species_z)'; 
  matrix[N_pid, 6  * (K - 1)] pid_v = (diag_matrix(sigma_pid) * pid_z)';
  
  matrix[N_stimuli, (K - 1)] stimulus_v = (diag_matrix(sigma_stimulus) * stimulus_z)'; 
  matrix[N_fs, (K - 1)] fs_v = (diag_matrix(sigma_fs) * fs_z)'; 
  matrix[N_species*N_fs, (K - 1)] fs_species_v = (diag_matrix(sigma_fs_species) * fs_species_z)'; 
  ...
  for (i in 1:N) {
      ...
      mu[i, 2] = b[stimulus_cat[i], 2] + species_v[species_id[i], 7 + 1] + species_v[species_id[i], 7 + 1 + stimulus_cat[i]] + pid_v[pid[i], 6 + 1] + stimulus_v[stimulus_id[i], 2] + fs_v[fs_id[i], 2] + fs_species_v[fs_species_id[i], 2] + (b[4, 2] + species_v[species_id[i], 7 + 5] + pid_v[pid[i], 6 + 5])*log_AP_size[i] + (b[5, 2] + species_v[species_id[i], 7 + 6] + pid_v[pid[i], 6 + 6])*AP_md[i];
