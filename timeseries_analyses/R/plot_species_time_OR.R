plot_species_time_OR <- function(preds) {
  ...
  pred_time_long_OR_summary <- pred_time_long %>% 
    ...
    mutate(est = log( (Agent/(1 - Agent)) / (Patient/(1 - Patient))) ) %>% 
  ...
  ## Export estimates #####################
  write_csv(pred_time_long_OR_summary %>% ungroup() %>% dplyr::select(-c(condition, upper_50_1, upper_50_2, upper_90_1, upper_90_2, lower_50_1, lower_50_2, lower_90_1, lower_90_2)), file = "pred_time_series_LOR.csv")
  ...
  p_adult <-  ggplot(filter(pred_time_long_OR_summary, Species == "Human"), aes(x = time_c, y = med)) + 
    ...
    geom_ribbon(aes(ymin = lower_50_1, ymax = upper_50_1, x = time_c), alpha = 0.4, fill = "#F9A630", color = NA) +
    geom_ribbon(aes(ymin = lower_50_2, ymax = upper_50_2, x = time_c), alpha = 0.4, fill = "#b22222", color = NA) +
    geom_ribbon(aes(ymin = lower_90_1, ymax = upper_90_1, x = time_c), alpha = 0.4, fill = "#F9A630", color = NA) +
    geom_ribbon(aes(ymin = lower_90_2, ymax = upper_90_2, x = time_c), alpha = 0.4, fill = "#b22222", color = NA) +
  ...
  p_infant <-  ggplot(filter(pred_time_long_OR_summary, Species == "Human baby"), aes(x = time_c, y = med)) + 
    facet_grid(~ stimulus_cat, scales = "free_y", space = "free_y") +
    geom_hline(aes(yintercept = 0), linetype = "dashed") +
    geom_ribbon(aes(ymin = ifelse(lower_50 > 0, NA, lower_50), ymax = ifelse(upper_50 < 0, upper_50, 0), x = time_c), alpha = 0.4, fill = "#F9A630", color = NA) +
    geom_ribbon(aes(ymin = ifelse(lower_50 > 0, lower_50, 0), ymax = ifelse(upper_50 > 0, upper_50, NA), x = time_c), alpha = 0.4, fill = "#b22222", color = NA) +
    geom_ribbon(aes(ymin = ifelse(lower_90 > 0, NA, lower_90), ymax = ifelse(upper_90 < 0, upper_90, 0), x = time_c), alpha = 0.4, fill = "#F9A630", color = NA) +
    geom_ribbon(aes(ymin = ifelse(lower_90 > 0, lower_90, 0), ymax = ifelse(upper_90 > 0, upper_90, NA), x = time_c), alpha = 0.4, fill = "#b22222", color = NA) +
    geom_line(lwd = 1.3, alpha = 0.75) +
  ...
  p_chimp <-  ggplot(filter(pred_time_long_OR_summary, Species == "Chimpanzee"), aes(x = time_c, y = med)) + 
    ...
    ylab("Log Odds of Agent vs. Patient Fixation") +
    xlab("") +
    AOI_col_scale +
    AOI_fill_scale +
    scale_linetype_manual(values = c("solid", "dotted", "dashed")) +
  ...
  ggsave("time_course_LOR.pdf", plot = p, height = 14, width = 13)
