plot_species_time_OR_AO <- function(preds) {
  ...
  ## Export estimates #####################
  write_csv(pred_time_long_OR_summary %>% ungroup() %>% dplyr::select(-c(condition, upper_50_1, upper_50_2, upper_90_1, upper_90_2, lower_50_1, lower_50_2, lower_90_1, lower_90_2)), file = "pred_time_series_LO_AO.csv")
  ...
  p_adult <-  ggplot(filter(pred_time_long_OR_summary, Species == "Human"), aes(x = time_c, y = med)) + 
    ...
    geom_ribbon(aes(ymin = lower_50_1, ymax = upper_50_1, x = time_c), alpha = 0.4, fill = "darkgrey", color = NA) +
    geom_ribbon(aes(ymin = lower_50_2, ymax = upper_50_2, x = time_c), alpha = 0.4, fill = "#b22222", color = NA) +
    geom_ribbon(aes(ymin = lower_90_1, ymax = upper_90_1, x = time_c), alpha = 0.4, fill = "darkgrey", color = NA) +
  ...
  p_chimp <-  ggplot(filter(pred_time_long_OR_summary, Species == "Chimpanzee"), aes(x = time_c, y = med)) + 
    facet_grid(~ stimulus_cat, scales = "free_y", space = "free_y") +
    geom_hline(aes(yintercept = 0), linetype = "dashed") +
    geom_ribbon(aes(ymin = ifelse(lower_50 > 0, NA, lower_50), ymax = ifelse(upper_50 < 0, upper_50, 0), x = time_c), alpha = 0.4, fill = "darkgrey", color = NA) +
    ...
    geom_ribbon(aes(ymin = ifelse(lower_90 > 0, NA, lower_90), ymax = ifelse(upper_90 < 0, upper_90, 0), x = time_c), alpha = 0.4, fill = "darkgrey", color = NA) +
  ...
  ggsave("time_course_LO_AO.pdf", plot = p, height = 14, width = 13)
